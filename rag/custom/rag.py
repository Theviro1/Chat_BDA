...
class CustomRAG:
    def __init__(self, llm:BaseLanguageModel, embedding:Embed, reranker:Rerank, templates):
        ...
        self.logger_handler = RAGLogger()
        
    
    def rag_pre_process(self, file_path:str):
        self.logger_handler.info('executing RAG pre-process...')
        # 进行文档分析
        file_name = os.path.basename(file_path)
        self.logger_handler.info('chunking file...')
        docs, file_name, chunk_ids = self.file_handler.handle(file_path)
        # 大模型预处理
        self.logger_handler.info('using LLM to transform sentences...')
        contents = self.transform_handler.transform_documents(docs)
        # embed
        self.logger_handler.info('embedding vectors...')
        embeddings = self.embeddings_handler.embedding(contents)
        # 存入milvus数据库
        self.logger_handler.info('inserting into milvus...')
        self.milvus_handler.insert(file_name=file_name, chunk_ids=chunk_ids, embeddings=embeddings, contents=contents)
        self.logger_handler.info('pre-process finished successfully.')
    ...
    def rag_post_process(self, query:str, distance_threshold:float=DISTANCE_THRESHOLD, score_threshold:float=SCORE_THRESHOLD, num_threshold:int=NUM_THRESHOLD)->List[str]:
        self.logger_handler.info('executing RAG post-process...')
        # 对问题进行预处理
        query = self.transform_handler.transform_query(query)
        # 检索向量库相似向量
        self.logger_handler.info('searching milvus database...')
        ...
        # rerank
        self.logger_handler.info('reranking results...')
        ...
        self.logger_handler.info('post-process finished successfully.')
        return r
    ...
    def list_file(self):
        r = self.milvus_handler.list_file()
        r = list(set(r))
        return r